...
You can also set a global default model via env and omit `model` in code:

```bash
STAGEHAND_MODEL_NAME=newapi/gpt-4.1-mini
```
...
<Tab title="NewAPI / OpenAI-compatible Proxy">

For OpenAI-compatible relay services such as NewAPI, use the `newapi/` provider prefix and point `baseURL` at the relay's OpenAI-compatible `/v1` endpoint:

```typescript
const stagehand = new Stagehand({
  env: "BROWSERBASE",
  model: {
    modelName: "newapi/gpt-4.1-mini",
...
You can also rely on env auto-loading:

```bash
NEWAPI_API_KEY=your_newapi_key
NEWAPI_BASE_URL=https://your-newapi-domain.com/v1
```
